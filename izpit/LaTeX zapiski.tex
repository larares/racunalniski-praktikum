...
\pgfplotsset{compat=1.18} %to so mi nekje predlagal, da vklopim, ne vem, če je nujno potrebno
...
\makeindex
%ukaz, ki naredi stvarno kazalo, a ga ne prikaže (mora biti v preambuli) (ta ukaz mi ni naredil stvarnega kazala???)
...
% naslov (včasih je lahko to tudi pred begin document)
...
\subsubsection{Pravila za pisanje izrazov v prikaznem načinu:}
...
\section{Razdelki in podrazdelki} 
Članek lahko hierarhično razdelimo na razdelke %(\section), 
podrazdelke, %(\subsection), 
podpodrazdelke in %(\subsubsection)
podpodpodrazdelke. %(\paragraph). 
V večini primerov potrebujemo samo razdelke in podarazdelke.
...
\subsection{Ukazi s katerimi nastavimo velikost pisave, a tega raje ne počnite na roke}
\begin{center}
{\Huge Zdravljica} \\
...
\subsection{Neoštevilčeno naštevanje:}
%
\begin{itemize}
...
\subsection{Oštevilčeno naštevanje:}
%
\begin{enumerate}
...
\subsection{Vgnezdeno naštevanje:}
%
\begin{enumerate}
\item bla
\item blabla
...
\section{VKLJUČEVANJE SPLETNIH NASLOVOV}
\begin{itemize}
...
\section{CITIRANJE}
V {\LaTeX}u citiramo s pomočjo Bibtexa
...
Nujno mora biti to na koncu!!
Pogosta napaka je, da uporabite samo ukaza bibliographystyle in bibliography, ne da bi uporabili ukaz cite. Ker LaTeX v seznamu literature izpiše samo literaturo, na katero se skličete v besedilu, se v takem primeru ne izpiše nič.
...
% nastavimo stil za bibliografije (plain, alpha, abbrv,...)

\bibliography{literatura}
%datoteka za Bibtex


% Vsa navodila bodo privzela, da razumete razliko med okolji in ukazi, ter da veste, kateri ukazi spadajo v preambulo, kateri pa v vsebino dokumenta. Podobno navodila privzemajo, da veste, kaj so parametri okolij in ukazov (v {}) in kako izgledajo neobvezni pomožni parametri ([]).

% V preambuli morate:

% z \documentclass ustrezno določiti razred article ali beamer ter nastaviti velikost papirja in pisave (a4paper, 10pt, 14pt, …),
% z \usepackage našteti ustrezne pakete,
% s paketi babel, inputenc in fontenc nastaviti ustrezno kodiranje ter podporo za slovenščino,
% vedeti, kdaj potrebujete pakete amsmath, amssymb, amsthm, hyperref, booktabs, graphicx, tikz, pgfplots ter naložiti samo tiste, ki jih potrebujete,
% z \newcommand definirati nove ukaze,
% z \newtheorem definirati nova okolja za definicije in trditve (vključno z ustrezno uporabo \theoremstyle).
% V dokumentu morate znati vključiti:

% naslov: \title, \author, \date, \maketitle,
% dodatne datoteke: \input,
% povzetek in kazalo: abstract, \tableofcontents,
% razdelke: \section, \subsection, …
% oblikovano besedilo: \emph, \textbf, \textit, \texttt, \verb, \url, \noindent, \centering, center, verbatim,
% sezname (vključno z gnezdenimi): itemize, enumerate in \item,
% tabele: table, tabular, \hline, \toprule, \midrule, \bottomrule, \multicolumn,
% slike: figure, \includegraphics, tikzpicture (samo za že pripravljene slike),
% sklice: label, ref, eqref, pageref,
% citate: cite, nocite, bibliographystyle, bibliography.
% Pri predstavitvah morate znati uporabiti:

% temo z ukazom \usetheme (navodila bodo določila ime teme),
% nove okvirje frame, \titlepage,
% bloke: block, exampleblock in alertblock,
...
% Pri matematiki morate ločiti med prikaznim (\[…\]) in vrstičnim načinom (\(…\) ali $…$) ter vedeti, kdaj uporabiti katerega. Brez dodatnih razlag morate poznati:
